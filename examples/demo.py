...
TRUE = np.array([0.3, -0.2, 0.4, -0.5, 0.1])          # hidden misfit
...
def simulate(theta):
    ...
    trip = 8.0 + 1.5 * z[0]
    purpose = np.array([0.5, 0.3, 0.2]) + 0.15 * np.array([z[1], -z[1], z[2] - z[2]])
    mode = np.array([0.45, 0.2, 0.25, 0.1]) + 0.2 * np.array([z[3], -z[3] / 2, -z[3] / 2, 0]) + 0.1 * np.array([z[4], 0, -z[4], 0])
...
def main():
    ...
    print("expected: composite rises from ~55-70 to >95; total run time well under a minute.")
